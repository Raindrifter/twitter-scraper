...
func (s *Scraper) FetchTweetsByUserID(userID string, maxTweetsNbr int, cursor string) ([]*Tweet, string, error) {
	...
	req, err := s.newRequest("GET", "https://twitter.com/i/api/graphql/eS7LO5Jy3xgmd3dbL044EA/UserTweets")
	if err != nil {
		return nil, "", err
	}
...
// FetchTweetsByUserID gets tweets for a given userID, via the Twitter frontend GraphQL API.
func (s *Scraper) FetchLikesByUserID(userID string, maxTweetsNbr int, cursor string) ([]*Tweet, string, error) {
	if maxTweetsNbr > 200 {
		maxTweetsNbr = 200
	}

	req, err := s.newRequest("GET", "https://twitter.com/i/api/graphql/IohM3gxQHfvWePH5E3KuNA/Likes")
	if err != nil {
		return nil, "", err
	}
